...
<tr id="invoice_{{ $invoice->bill_invoiceid  }}">
    ...
    @if(request()->input('user_role_type') == 'admin_role' || request()->input('user_role_type') == 'franchise_admin_role')
        <td class="invoices_col_user">
            <img src="{{ getUsersAvatar($invoice->avatar_directory, $invoice->avatar_filename) }}" alt="user"
        class="img-circle avatar-xsmall"> 
        {{ $invoice->creator->first_name }} {{ $invoice->creator->last_name }} ({{ $invoice->creator->email }})
        </td>
    @endif
